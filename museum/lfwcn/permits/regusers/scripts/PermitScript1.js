...
function checkLatLong(value)
{
	var latlong = /(^\d{1,3}\.\d+$)/;
	if(latlong.test(value))
		return true;
...
function validateform()
{
	...
	var county = document.getElementById('parish');
	...
	{
		errorText += '<br>no parish entered.';
		submit = false;
	}
...
function checkRegForm()
{
	...
	function enablePwdField()
	{
	    
		if(document.getElementById('pwdbox').checked)
		
		{
			document.getElementById('pwd').disabled = false;
			document.getElementById('pwdRepeat').disabled = false;							
		}
		
	    else
	    {
			document.getElementById('pwd').disabled = true;
			document.getElementById('pwdRepeat').disabled = true;
	    }	
	}
